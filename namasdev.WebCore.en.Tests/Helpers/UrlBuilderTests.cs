...
namespace namasdev.WebCore.Tests.Helpers
{
    public class UrlBuilderTests
    {
        ...
        [Fact]
        public void BuildOrderExpression_NewColumn_ReturnsOrder()
        {
        ...
        [Fact]
        public void BuildOrderExpression_SameAscColumn_ReturnsDesc()
        {
        ...
        [Fact]
        public void BuildOrderExpression_SameDescColumn_ReturnsAsc()
        {
        ...
        [Fact]
        public void BuildOrderExpression_MultiColumn_AllDesc()
        {
            var result = UrlBuilder.BuildOrderExpression("a,b", "a,b");
            Assert.Equal("a desc,b desc", result);
        }

        [Fact]
        public void BuildOrderExpression_MultiColumn_ApplyDescToFirstOnly()
        {
            var result = UrlBuilder.BuildOrderExpression("a,b", "a,b", applyOrderDescToFirstElementOnly: true);
            Assert.Equal("a desc,b", result);
        }

        [Fact]
        public void BuildOrderExpression_MultiColumn_DescToAsc()
        {
            var result = UrlBuilder.BuildOrderExpression("a,b", "a desc,b desc");
            Assert.Equal("a,b", result);
        }

        // ── BuildUrlWithParameter (HttpRequest) ───────────────────────────────

        [Fact]
        public void BuildUrlWithParameter_NullRequest_Throws()
        {
            Assert.Throws<ArgumentNullException>(
                () => UrlBuilder.BuildUrlWithParameter(null!, "page", "1"));
        }

        [Fact]
        public void BuildUrlWithParameter_SetsNewParam()
        {
            var request = BuildRequest("/list");
            var result = UrlBuilder.BuildUrlWithParameter(request, "foo", "bar");
            Assert.Equal("/list?foo=bar", result);
        }

        [Fact]
        public void BuildUrlWithParameter_NullValue_RemovesParam()
        {
            var request = BuildRequest("/list", "?foo=bar");
            var result = UrlBuilder.BuildUrlWithParameter(request, "foo", null);
            Assert.Equal("/list", result);
        }
        ...
        [Fact]
        public void BuildAbsoluteUrl_ReturnsSchemeHostAndPath()
        {
            var request = BuildRequest("/items/1");
            var result = UrlBuilder.BuildAbsoluteUrl(request, "/items/1");
            Assert.Equal("https://example.com/items/1", result);
        }
    }
}
